...
bool TickValid(int tick) {
	float delta = 0 - (TICKS_TO_TIME(INTERFACES::Globals->tickcount) - TICKS_TO_TIME(tick));
...
void CBacktrack::run_legit(SDK::CUserCmd* cmd) //phook backtrack muahhahahahaaha
{
	...
	for (int i = 1; i < 65; i++)
	{
		...
		float simtime = entity->GetSimTime(); Vector hitboxPos = aimbot->get_hitbox_pos(entity, 0);
		Vector oriboy = entity->GetVecOrigin();
		Vector angs = entity->GetAbsAnglesVec();
		headPositions[i][cmd->command_number % 12] = legit_backtrackdata{ simtime, hitboxPos ,oriboy ,angs , entity->GetFlags() };
