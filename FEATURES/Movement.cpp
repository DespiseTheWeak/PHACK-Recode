...
void CMovement::quick_stop(SDK::CUserCmd* cmd)
{
	for (int i = 1; i <= 65; i++)
	{
		...
		//if (AutoWall->GetDamagez(local_position, aimbot->get_hitbox_pos(entity, aimbot->scan_hitbox(entity)), local_player, entity).damage > SETTINGS::settings.damage_val) //autowall to the enemies position and see if they meet minimum damage
		//{
		...
		{
			if (AutoWall->GetDamagez(entity->GetBonePosition(hitboxes[i])))
			{
				cmd->move.x = 450; //because fuck you
