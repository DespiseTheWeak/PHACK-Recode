...
void CFakeLag::do_fakelag(SDK::CUserCmd* cmd)
{
	...
	if (!net_channel)
		return;
//	if (SETTINGS::settings.disable_lag)
//		return;

		if (SETTINGS::settings.lag_bool)
		{
			if (SETTINGS::settings.smart_lag)
			{
				SETTINGS::settings.jump_lag = (rand() % 30);
				SETTINGS::settings.move_lag = 13;
			}
			if (fake_walk) {
				if ((local_update - INTERFACES::Globals->curtime) > 0.22) {
					choke_amount = SETTINGS::settings.fakwal;
				}
				else {
					choke_amount = 1;
				}
